...
class DetectionDomainServiceV3:
    ...
    def _run_ocr_once(self, image_path: str, ocr_reader: Any) -> None:
        ...
        if engine is not None:
            self._cached_global_feat = engine.extractor.extract_global_feature(img_cv2)
        ...
        try:
            ...
            for b in bboxes:
                ...
                try:
                    ...
                    async with self.semaphore:
                        res_str = await run_in_threadpool(engine.predict, image_path, b_list, "xyxy", global_feat)
                    if await self._is_canceled(task_id):
                        return
